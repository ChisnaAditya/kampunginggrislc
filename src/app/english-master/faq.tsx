...
export default function Faq() {
  const faqContent = [
    {
      title:
        "Berapa usia minimal untuk join program english master dan english master plus?",
    ...
    {
      title:
        "Fasilitas apa saja yang didapat di program english master dan english master plus?",
      content:
    ...
    {
      title:
        "Di mana lokasi gedung belajar dan asrama untuk Program English Master?",
      content:
    ...
    {
      title:
        "Apa perbedaan program intensive dengan EM 4 bulan atau EMP 6 bulan?",
      content:
  ...
  function Accordion(prop: accordionProps) {
    ...
    return (
      <div
        onClick={handleOpen}
        className="border-[0.5px] border-[#232323] p-4 rounded-xl"
      >
        <div className="flex items-center justify-start gap-4 lg:px-4 lg:py-2 cursor-pointer">
          <div>
            <IconPlus />
          </div>
          <p className="text-md leading-tight lg:text-[22px] font-bold ">
            {HTMLReactParser(prop.title)}
          </p>
        </div>
        {isActive && (
