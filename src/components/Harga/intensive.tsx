  ...
  return (
    <div className="flex flex-col items-center lg:flex-row lg:items-start gap-[20px] p-2">
      {harga.map((item, index) => (
        <div
          key={index}
          className="flex flex-col justify-between w-full  rounded-xl shadow-lg bg-white p-4 overflow-hidden"
        >
          <div className="relative">
            {item.populer && (
              <div className="absolute top-[10px] left-[110px] transform rotate-45 text-center bg-secondaryLC shadow-lg p-1 w-full">
                POPULER
              </div>
            )}
            ...
            <div className="flex flex-col items-center py-2 my-2 border-[1px]">
              {item.prices.map((price, indexx) => (
                <div
                  key={indexx}
                  className="flex items-center justify-between px-2 gap-10 w-full "
                >
                  <p>{price.duration}</p>
